...
export async function updateRequest(req, res, next) {
  try {
    ...
    // Handle file uploads to Cloudinary (similar to createRequest)
    const uploadedAttachments = [];
    const files = req.files || [];
    ...
    // Merge new attachments with existing ones (or replace if specified)
    ...
    if (uploadedAttachments.length > 0) {
      ...
      // If attachments are provided in body (JSON), use them
      let bodyAttachments = req.body.attachments;
      if (typeof bodyAttachments === 'string') {
        try {
          bodyAttachments = JSON.parse(bodyAttachments);
      ...
      if (Array.isArray(bodyAttachments)) {
        attachments = bodyAttachments.filter(a => a && typeof a === 'object' && (a.url || a.name));
      }
    }
    
    // Clean attachments format
    const finalAttachments = attachments.map(att => ({
      name: String(att.name || ''),
      url: String(att.url || ''),
      type: String(att.type || 'file')
    ...
    const description = req.body?.description || doc.description;
    const budget = req.body?.budget !== undefined ? parseFloat(req.body.budget) : doc.budget;
    const deadline = req.body?.deadline ? new Date(req.body.deadline) : doc.deadline;
    
    // If request was rejected, set status back to 'pending' for resubmission
    const newStatus = doc.status === 'rejected' ? 'pending' : (req.body?.status || doc.status);
    
    doc.title = title;
    doc.description = description;
    doc.budget = budget;
    doc.deadline = deadline;
    doc.attachments = finalAttachments;
    doc.status = newStatus;
    doc.rejectionReason = ''; // Clear rejection reason when resubmitting
    ...
    const notification = await Notification.create({
      user: req.user.sub,
      title: doc.status === 'pending' ? "Request Resubmitted" : "Request Updated",
      Message: doc.status === 'pending' 
        ? `Your request "${title}" has been resubmitted and is pending admin review.`
